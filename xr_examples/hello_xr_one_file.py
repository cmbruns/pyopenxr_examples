...
def main():
    ...
    for layer in layers:
        ...
        extension_properties = xr.enumerate_instance_extension_properties(layer.layer_name.decode())
        logger.debug(f"    Available Extensions ({len(extension_properties)})")
        for extension in extension_properties:
            logger.debug(
                f"      Name={extension.extension_name.decode()} SpecVersion={extension.extension_version}")
    ...
    with ExitStack() as exit_stack:  # noqa
        ...
        for b in [xr.EnvironmentBlendMode.ALPHA_BLEND, xr.EnvironmentBlendMode.OPAQUE]:
            if b in blend_modes:
                ...
                break
        if environment_blend_mode is None:
            environment_blend_mode = blend_modes[0]
        print(f"blend mode = {xr.EnvironmentBlendMode(environment_blend_mode).name}")  # TODO box/unbox enum fields
